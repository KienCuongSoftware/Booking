    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div class="block mt-4">
            <label for="remember_me" class="inline-flex items-center">
                <input id="remember_me" type="checkbox" class="rounded-md border-gray-300 text-bcom-blue shadow-sm focus:ring-bcom-blue/40 focus:ring-2" name="remember">
        ...
        <div class="flex items-center justify-end mt-6 gap-3 flex-wrap">
            @if (Route::has('password.request'))
                <a class="text-sm text-bcom-blue hover:text-bcom-navy font-medium rounded-lg focus:outline-none focus:ring-2 focus:ring-bcom-blue/30 px-1" href="{{ route('password.request') }}">
                    {{ __('Quên mật khẩu?') }}
                </a>
            @endif
