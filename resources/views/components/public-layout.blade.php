                ...
                <nav class="flex flex-wrap items-center justify-end gap-1 text-sm sm:gap-2" aria-label="{{ __('Điều hướng chính') }}">
                    ...
                    @auth
                        @if (auth()->user()->role->value === 'customer')
                            <a href="{{ route('customer.bookings.index') }}" class="rounded-lg px-3 py-2 font-medium text-white/90 hover:bg-white/10 hover:text-white">{{ __('Đơn đặt của tôi') }}</a>
                        @else
                        ...
                        @endif
                        <a href="{{ route('profile.edit') }}" class="rounded-lg px-3 py-2 font-medium text-white/90 hover:bg-white/10 hover:text-white">{{ __('Hồ sơ') }}</a>
                        <form method="POST" action="{{ route('logout') }}" class="inline">
                            @csrf
                            <button type="submit" class="rounded-lg px-3 py-2 font-medium text-white/90 hover:bg-white/10 hover:text-white">{{ __('Đăng xuất') }}</button>
                        </form>
                    @else
