<header class="sticky top-0 z-40 border-b border-bcom-navy-dark/40 bg-bcom-navy shadow-md" x-data="customerInboxBadge()">
    <div class="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 px-4 py-3 sm:px-6 lg:px-8">
        ...
        <nav class="flex flex-wrap items-center justify-end gap-1 text-sm sm:gap-2" aria-label="{{ __('Điều hướng chính') }}">
            ...
            <a href="{{ route('customer.bookings.index') }}"
                class="relative inline-flex items-center rounded-lg px-3 py-2 font-medium {{ request()->routeIs('customer.bookings.*') && ! request()->routeIs('customer.favorites.*') ? 'bg-white/15 text-white' : 'text-white/90 hover:bg-white/10 hover:text-white' }}">
                {{ __('Đơn đặt của tôi') }}
                <span
                    x-show="unread > 0"
                    x-cloak
                    class="absolute -right-0.5 -top-0.5 flex h-4 min-w-[1rem] items-center justify-center rounded-full bg-bcom-yellow px-1 text-[10px] font-bold leading-none text-bcom-navy"
                    x-text="unread > 9 ? '9+' : unread"
                ></span>
            </a>
            <a href="{{ route('customer.reviews.index') }}"
                class="rounded-lg px-3 py-2 font-medium {{ request()->routeIs('customer.reviews.*') ? 'bg-white/15 text-white' : 'text-white/90 hover:bg-white/10 hover:text-white' }}">
                {{ __('Đánh giá') }}
            </a>
