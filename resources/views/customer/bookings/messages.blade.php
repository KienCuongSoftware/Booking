<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-wrap items-center justify-between gap-2">
            ...
            <a href="{{ route('customer.bookings.show', $booking) }}" class="text-sm font-medium text-bcom-blue hover:underline">{{ __('← Chi tiết đơn') }}</a>
        </div>
    </x-slot>
    <div class="py-8 px-4 sm:px-6 lg:px-8">
        <div class="mx-auto max-w-2xl space-y-4">
            ...
            <form method="POST" action="{{ route('customer.bookings.messages.store', $booking) }}" class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm space-y-2">
                @csrf
                <textarea name="body" rows="3" class="w-full rounded-xl border-gray-200 text-sm" required placeholder="{{ __('Nhập nội dung…') }}">{{ old('body') }}</textarea>
