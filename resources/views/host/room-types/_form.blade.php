...
<form method="POST" action="{{ $action }}" enctype="multipart/form-data" class="space-y-6">
    ...
    <p class="text-sm text-gray-600">
        {{ __('Khách sạn:') }} <span class="font-semibold text-red-800">{{ $hotel->name }}</span>
    </p>
    ...
    <div class="grid gap-5 md:grid-cols-2">
        ...
        @if ($amenitySections->isNotEmpty())
            <div class="md:col-span-2 space-y-6 rounded-xl border border-red-100 bg-red-50/20 p-5">
                <div>
                    ...
                    <div>
                        ...
                        <div class="mt-2 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                            @foreach ($amenities as $amenity)
                                <label class="flex items-start gap-2 rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-sm text-gray-800 hover:border-red-200 has-[:checked]:border-red-300 has-[:checked]:bg-red-50/40">
        ...
        <div class="md:col-span-2 rounded-xl border border-red-100 bg-red-50/30 p-4">
            ...
            <div class="mt-3 space-y-2" x-data="{ lines: @js($bedLinesInit) }">
                <template x-for="(line, index) in lines" :key="index">
                    ...
                    <div class="grid grid-cols-1 gap-2 rounded-lg border border-red-100 bg-white p-3 sm:grid-cols-[minmax(0,11rem)_minmax(0,1fr)_auto] sm:items-center sm:gap-3">
                        <input type="text"
                            class="block w-full min-w-0 rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm text-gray-900 shadow-sm placeholder:text-gray-400 focus:border-red-500 focus:outline-none focus:ring-2 focus:ring-red-500/20 [field-sizing:fixed]"
                        ...
                        <button type="button"
                            class="justify-self-start text-sm text-red-700 hover:underline sm:justify-self-end sm:whitespace-nowrap sm:px-1"
    ...
    <div class="flex flex-wrap items-center gap-3">
        ...
        <a href="{{ route('host.rooms.index', ['hotel_id' => $hotel->id]) }}" class="text-sm text-gray-600 hover:text-red-700">{{ __('Quay lại danh sách phòng') }}</a>
    </div>
</form>
